...
#                print(str(i)+':padding:'+str(len(outputs)))
                for _ in range(seq.size(1) - i -1):                                        
                    outputs.append(padding.clone())                 
                break                
            xt = self.embed(it)
            ...
            if i==1:
                padding = Variable(output.data.new(batch_size, self.vocab_size + 1).zero_())
            
#            pad_v = torch.rand(batch_size, self.vocab_size + 1)
#        pad_v = pad_v.type_as(it)
